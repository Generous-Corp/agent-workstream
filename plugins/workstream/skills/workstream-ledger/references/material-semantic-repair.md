...
After review fills the payload and immutable artifact binding, generate every
control identity/digest and the current deterministic remote slot without hand
calculation:
...
```json
{
  ...
  "payload": {
    ...
    "issue_graph_frontier": {
      "algorithm": "authenticated-root-children-relations-v1", "sha256": "..."
    },
...
control itself has no business semantics. Resume validates raw envelopes first,
then every repair binding/frontier, and only then evaluates effective next
action, blocker, obligations, and checkpoint state.

If apply returns `durable_partial_replay_required`, the control comment already
exists. Do not build another manifest or advance its revision. Rerun the exact
